...
class GetChunks:
    def __init__(self, fasta_file='', pkl_file='', chunksize=1998, overlap=0, out_dir = "./", cds_token = 100):
        self.fasta = fasta_file
        self.pkl = pkl_file
        self.chunksize = chunksize
        self.overlap = overlap
        self.out_dir = os.path.join(out_dir, "inter_intron_cds_chunks_" + str(self.chunksize))
...
#        self.chrom = "_".join(basename[2:4])        
    ...
    def get_chunks(self):
        ...
        for i in tqdm(range(num_chunks-1), desc = "Cut chr"):
            ...
            m = self.m[begin:end,:]
            chunk_m = coo_matrix(m)
            if np.all(m[:, 0] == 1):
                label = "intergenic"
            elif np.all(np.logical_or(np.logical_or(m[:, 1], m[:, 2]), m[:, 3])):
                label = "intron"
            elif np.sum(np.any(m[:, 4:] != 0, axis=1)) >= self.cds_token:
                label = "cds"
            else:
                label = "non"
...
#            sparse_matrix = coo_matrix(current_annotation['annotation'])
            ...
            with open(os.path.join(f'{self.out_dir}',\
                 f"{self.spec}_{self.chrom}_{begin}-{end}_{self.strand}_{label}.pkl"), 'wb') as file:
...
#    out_path = os.path.join(args.out_dir, "inter_intron_cds_chunks_" + str(args.chunk_size))
