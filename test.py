...
class GenomeSequences:
    ...
    def read_fasta(self):
        ...
        chr_dict = {key: record for key, record in fasta_dict.items() \
                    if record.description.split(",")[0].split(" ")[-2].lower() == "chromosome" and \
                    (record.description.split(",")[0].split(" ")[-1].isdigit() or \
                     record.description.split(",")[0].split(" ")[-1].isalpha()) \
                    } 
...
if __name__ == '__main__':
    ...
    files = os.listdir(fasta_out) if os.path.exists(fasta_out) else []
    if len(files) > 10:
