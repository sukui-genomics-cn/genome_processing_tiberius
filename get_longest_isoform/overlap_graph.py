...
# ==============================================================
from get_longest_isoform.features import Node_features
import numpy as np
...
class Graph:
    ...
    def compare_tx_cds(self, tx1, tx2):
        ...
        if not tx1.strand == tx2.strand:
            return False
        coords = []
        coords += [c + [int(phase)] if phase.isdigit() else c + [int(0)] for phase, coord_phase in
                   tx1.get_type_coords('CDS').items() for c in coord_phase]
        coords += [c + [int(phase)] if phase.isdigit() else c + [int(0)] for phase, coord_phase in
                   tx2.get_type_coords('CDS').items() for c in coord_phase]
