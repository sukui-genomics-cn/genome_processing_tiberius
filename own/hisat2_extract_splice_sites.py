...
def extract_splice_sites(gtf_file, anno_types):
    genes = dd(list)
    trans = {}
    ...
    for chrom, strand, exons in trans.values():
        if chrom not in exons_dict:
            exons_dict[chrom] = [*exons]
        else:
            exons_dict[chrom].extend(exons)
    ...
    for chrom, left, right, strand in junctions:
        if chrom not in intron_dict:
            intron_dict[chrom] = [(left+1, right-1, strand)]
        else:
            intron_dict[chrom].append((left+1, right-1, strand))
    return intron_dict, exons_dict
...
if __name__ == '__main__':
    ...
    parser.add_argument('gtf_file',
        ...
        default=r"D:\data\human\DNA_LLM\groups\Homo_sapiens\GCF_009914755.1_T2T-CHM13v2.0_genomic.gtf")
