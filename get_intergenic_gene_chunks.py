...
class GetChunks:
    def __init__(self, fasta_file='', pkl_file='', chunksize=1998, overlap=0, out_dir = "./", cds_token = 100):
        ...
        self.out_dir = os.path.join(out_dir, "inter_gene_chunks_" + str(self.chunksize))
...
#        self.chrom = "_".join(basename[2:4])        
    ...
    def get_chunks(self):
        ...
        for i in tqdm(range(num_chunks-1), desc = "Cut chr"):
            ...
            gene_token = np.sum(np.any(m[:, 1:] != 0, axis=1))
            fraction = round(float(gene_token/self.chunksize), 2)
#            sparse_matrix = coo_matrix(current_annotation['annotation'])
            ...
            with open(os.path.join(f'{self.out_dir}',\
                 f"{self.spec}_{self.chrom}_{begin}-{end}_{self.strand}_{fraction}.pkl"), 'wb') as file:
