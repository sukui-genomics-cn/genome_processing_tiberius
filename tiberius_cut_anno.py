    ...
    def get_flat_chunks(self, sequence_name=None, strand='+', coords=False, pad=True):
        ...
        if strand == '-':
            for i in range(len(chunks)):
                chunks[i] = chunks[i].translate(str.maketrans('ACGTacgt', 'TGCAtgca'))[::-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--length', type=int, help='length of chunks', default=19998)
    parser.add_argument('-ol', '--overlap', type=int, help='overlap length', default=0)
    ...
    parser.add_argument('-se', '--save_exon', action='store_true',
                        help='Save exon')
    parser.add_argument('-si', '--save_intergenic', action='store_true',
                        help='Save intergenic')
    ...
    fasta = GenomeSequences(fasta_file=args.fasta,
                            chunksize=args.length,
                            overlap=args.overlap)
    fasta.save_fasta(args, spec)

    f_chunk_fwd, chunk_coords_fwd = fasta.get_flat_chunks(strand='+', coords=True, pad=args.padding)
    f_chunk_bwd, chunk_coords_bwd = fasta.get_flat_chunks(strand='-', coords=True, pad=args.padding)

    save_chunks(f_chunk_fwd, chunk_coords_fwd, args, spec, 'forward')
    save_chunks(f_chunk_bwd, chunk_coords_bwd, args, spec, 'backward')

    ref_anno = GeneStructure(longest_anno_out,
                             chunksize=args.length,
                             overlap=args.overlap,
                             spec=spec)
    if args.save_exon:
        ref_anno.extract_exon_pkl(args, fasta.sequence_names, fasta.sequences, 2000)

    seqs_length = [len(s) for s in fasta.sequences]
    seq_names = fasta.sequence_names
    if args.type.lower() == 'tiberius':
        ref_anno.translate_to_one_hot_hmm_tiberius(seq_names, seqs_length, fasta.sequences, args)
        tiberius_chunks_fwd, tiberius_coords_fwd = ref_anno.get_flat_chunks_hmm_tiberius(fasta.sequence_names, strand='+', coords=True)
        tiberius_chunks_bwd, tiberius_coords_bwd = ref_anno.get_flat_chunks_hmm_tiberius(fasta.sequence_names, strand='-', coords=True)
        # save_chunks_hmm_tiberius(tiberius_chunks_fwd, tiberius_coords_fwd, args, spec, 'forward')
        # save_chunks_hmm_tiberius(tiberius_chunks_bwd, tiberius_coords_bwd, args, spec, 'forward')
        save_chunk_anno(args)

    elif args.type == '17':
        ref_anno.translate_to_one_hot_hmm(seq_names, seqs_length, fasta.sequences, args)
        save_chunk_anno(args)

    elif args.type == '4':
        ref_anno.translate_to_one_hot_4_labels(seq_names, seqs_length, fasta.sequences, args)
        save_chunk_anno(args)
